...
- (void)setLayers
{
    _parLayer = [CAReplicatorLayer layer];//复制图层
    _parLayer.frame = self.bounds;
    _parLayer.instanceCount = 2;
    _parLayer.instanceTransform = CATransform3DMakeRotation(M_PI, 0, 0, 1);//复制图层在被创建时产生的和上一个复制图层的变换(位移的锚点时CAReplicatorlayer的中心点) 以中心点为中心在Z轴上旋转180°
